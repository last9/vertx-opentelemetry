...
/**
 * Utility for adding OpenTelemetry tracing to Vert.x 4 Kafka batch consumers.
 *
 * <p>The Vert.x {@code VertxTracer} SPI instruments HTTP, EventBus, and database clients
 * automatically, but does <em>not</em> instrument Kafka consumer handlers. Without
 * instrumentation, {@link Span#current()} returns the no-op root span inside a batch handler,
 * so {@code trace_id} and {@code span_id} are empty in every log line produced during message
 * processing.
 *
 ...
 * capture the correct context via {@link RxJava3ContextPropagation}.
 *
 * <h2>Usage</h2>
 * <pre>{@code
